...
function checksExistsUserAccount(request, response, next) {
  ...
  if (!user) {
    return response.status(404).json({ error: "User name not found!" });
  }
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (!user.pro && user.todos.length >= 10) {
    return response.status(403).json({ error: "User with free plan is not allowed to save more information" });
  }
